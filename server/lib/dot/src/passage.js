...
function scorePassage(passage, queryWeights, idf, maxIdf) {
  ...
  for (const [t, qw] of queryWeights) {
    const w = idf.has(t) ? idf.get(t) : maxIdf

    // Only the words the user actually typed set the bar. Counting expanded
    // synonyms in the denominator meant a passage could match every typed word
    // and still score near zero, because "sign" alone pulls in a dozen
    // authentication synonyms that no single paragraph will ever contain — so
    // the passage that answered the question was never chosen.
    if (qw === 1.0) {
      total += w
