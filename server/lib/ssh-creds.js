...
function getForVendor(vendor) {
  ...
  for (const k of Object.keys(all)) {
    if (k === '__hosts__') continue; // reserved per-host map, not a vendor
    if (normVendor(k) === want) return all[k];
  }
  return all.default || null;
}

// Per-host credential overrides live under a reserved `__hosts__` key in the
// same encrypted blob: { "__hosts__": { "192.168.1.14": {username,password,...} } }.
// This lets two switches of the SAME vendor carry DIFFERENT passwords — e.g.
// .13 and .14 are both TP-Link JetStream but have distinct admin passwords.
// Lookup is exact-IP; callers fall back to getForVendor when there's no host entry.
function getForHost(host) {
  if (!host) return null;
  const all = getCreds();
  const map = all.__hosts__ || {};
  return map[String(host).trim()] || null;
}

// Store (or replace) the credentials for a single host. Re-encrypts the whole
// blob with the existing key and busts the cache. Throws if no key is configured.
function setForHost(host, creds) {
  if (!host) throw new Error('host required');
  const key = loadKey();
  if (!key) throw new Error('no SSH creds key configured (.env.key / SSH_CREDS_KEY)');
  const all = { ...getCreds() };
  const map = { ...(all.__hosts__ || {}) };
  const entry = { username: creds.username || '', password: creds.password || '' };
  if (creds.enablePassword) entry.enablePassword = creds.enablePassword;
  map[String(host).trim()] = entry;
  all.__hosts__ = map;
  writeEnvBlob(encrypt(JSON.stringify(all), key));
  clearCache();
  return true;
}
...
module.exports = {
  getCreds, getForVendor, getForHost, setForHost, clearCache,
  encrypt, decrypt,
