...
//   GET    /api/ports/:deviceId/:port/history    — current state + value at
//                                                  1h / 3h / 12h / 1d / 1w ago
//                                                  + recent change events
//   GET    /api/ports/:deviceId/:port/timeline   — snapshots for stacked-bar viz
//   POST   /api/ports/:deviceId/poll             — trigger a poll now
//                                                  (rate-limited; 409 if already polling)
//
...
// ── manual poll trigger ──────────────────────────────────────────────
// Per-user sliding window so a single client can't spam the SSH runner.
// The poller's own _busy guard also returns immediately on overlap, but
// returning 409 makes that visible to the caller. Rate window applies
// per (userId or IP) so multiple users can each trigger their own polls.
const POLL_RATE_WINDOW_MS = 60_000;
const POLL_RATE_MAX       = 5;
const _pollHits = new Map();

function pollRateOk(key) {
  const now = Date.now();
  const hits = _pollHits.get(key) || [];
  // Drop hits older than the window.
  const pruned = hits.filter((t) => t > now - POLL_RATE_WINDOW_MS);
  pruned.push(now);
  _pollHits.set(key, pruned);
  // Cheap GC: if the map grows beyond ~1000 keys, drop the oldest.
  if (_pollHits.size > 1000) {
    const oldest = [..._pollHits.keys()].slice(0, _pollHits.size - 1000);
    for (const k of oldest) _pollHits.delete(k);
  }
...
router.post('/api/ports/:deviceId/poll', safeAsync(async (req, res) => {
  ...
  if (!pollRateOk(rateKey)) {
    return res.status(429).json({ error: 'rate limit: max 5 manual polls per minute' });
  }
  if (poller.isBusy(id)) {
