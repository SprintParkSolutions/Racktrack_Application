...
| [approvals-test-sheet.html](features/approvals-test-sheet.html) | The tester's checklist for the drift approval workflow, 18 Sep 2026: 38 numbered checks in the order a real day goes, each with what to do, what should happen and a screenshot of the screen, grouped as the technician on the phone, the whole path in Approvals, the screens around it, a phone, and the things that must be refused; plus the accounts to sign in with and what is known and not a bug. [Artifact](https://claude.ai/artifact/W7tTiEGDXDUJFRQZX87eVD) |
