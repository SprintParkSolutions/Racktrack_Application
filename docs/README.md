...
| [racktrack-tester-guide.html](user-guide/racktrack-tester-guide.html) | For the internal test team: every screen of the app and of RackTrack Changes, six runs end to end, the words on the screens, the demo accounts (passwords blanked here, they are in TESTER-CREDENTIALS.md) and what is already known. Screenshots from the live demo, 21 September 2026. [Artifact](https://claude.ai/artifact/Wj35jFxxDwJXk2unWhRQWS) |
...
| Document | What it is |
|---|---|
| [racktrack-demo-simple-run.html](demo/racktrack-demo-simple-run.html) | The demo as one run, 22 September 2026: five acts and twenty-four beats, fourteen minutes, one paragraph to say at each stop. Covers every screen in the product - setup, the photograph, the shelves, the live switch reading, Review, the report, the drift check, port lookup, two racks, the Drift Desk from triage to approval, scan history and Ask DOT - plus a ninety second problem statement to open with, a five minute cut, the questions you will get and what to do if a screen fails. [Artifact](https://claude.ai/artifact/5DkgaFLWP34Hgokx1DZBpQ) |
| [racktrack-demo-script.html](demo/racktrack-demo-script.html) | The run book for Monday 21 September 2026 on the office rack SP-HYB-RM01-R01-R1, second edition: 28 sections from an empty organisation to the close, every step with a real capture in a device frame, numbered rings matched to the "Point at" lines, the clicks, the words, what to watch for, the live state on the day and what not to show. Screenshots in [img/](demo/img/), builder in [source/](demo/source/). [Artifact](https://claude.ai/artifact/4o9RdxnfuUyAaAwqwkxrAN) |
| [racktrack-demo-run-2026-09-23.html](demo/racktrack-demo-run-2026-09-23.html) | The demonstration as a run book: eight acts with a clock, what to do, the exact words to read out loud, and every screen captured off demo.racktrack.ai on 23 September 2026 (Android 82 / iOS 4). Built by [source/build-demo-run.py](demo/source/build-demo-run.py). [Artifact](https://claude.ai/artifact/3TyG7sppzwWMLZZbhUXKxK) |
| [racktrack-demo-pitch.html](demo/racktrack-demo-pitch.html) | The spoken pitch for the same demo: seven beats, about fifteen minutes, the words in order with what is on screen while they are said, a two-minute version, answers for push-back and the lines not to say. [Artifact](https://claude.ai/artifact/Th3qDk8aZfZgqD4fvD9JMF) |
...
| Document | What it is |
|---|---|
| [racktrack-redesign/](design/racktrack-redesign/) | The redesign after the 2 out of 10 on UI and UX (22 Sep 2026): twelve screens in one design language - the technician's guided check in six steps, the SPOC's Assigned to me and check review, the admin's Needs attention, and the shared parts. Built by gen.py from one set of tokens. [Artifact](https://claude.ai/artifact/U3ZUmoLZE9g1PvpPqANSKF) |
| [role-navigation-matrix.html](design/role-navigation-matrix.html) | What each role sees and where: the bottom bar, the four marks on Home and the Menu (which is also the desktop sidebar), for the employee, the single point of contact and the admin. Editable - every feature is in a dropdown, and the page copies the matrix back as text. [Artifact](https://claude.ai/artifact/8N8xvVubyNDeiSLLGXMQEj) |
...
| [report-drift-check-drift-desk.html](features/report-drift-check-drift-desk.html) | A three minute client walkthrough of the three things a customer sees in order: what the scan report contains (the camera and the switch as two witnesses, the four sections on screen, and the printed report's four tiers of verdict, what to do, the facts and what could not be checked), what the drift check does and which problem it solves (the record against the rack, how the rack is identified, the three kinds of difference, and the technician's single Send button), and the Drift Desk in detail (who decides what, the one check page and its tabs, the ten rules the server enforces, the queues and reports around it, and where ServiceNow fits). [Artifact](https://claude.ai/artifact/RCweBiXtyMMqW8mR7BE8rD) |
...
| [datacenter-setup-review.html](features/datacenter-setup-review.html) | The first-run datacenter setup as it is built on 21 Sep 2026, made for the owner to mark up before it is simplified: who is sent into it and when, how the popup behaves, all ten steps with every box, list and button (what is mandatory, what is pre-filled, and whether the rest of the app reads it or it is saved only), the Organization settings page, and nine overlaps found in the code. Every line has Keep, Remove and Change, every step has a notes box, and one button copies the marks back as plain text. [Artifact](https://claude.ai/artifact/TMVckUuBzxw8SXLUbgDdGX) |
...
| [field-mapping.html](netbox/field-mapping.html) | **Generated, do not edit by hand.** Every field RackTrack holds, what NetBox calls it, what ServiceNow calls it, and how each side stores it (direct field, reference, relationship row, custom u_ column, or not supported). 104 fields across all 16 object types. Printed from `server/lib/netbox/model.js`, `mapping.js` and `cmdb.js` by [build-field-mapping.js](../server/scripts/build-field-mapping.js); regenerate with `node server/scripts/build-field-mapping.js`. Same rows as a spreadsheet: [field-mapping.csv](netbox/field-mapping.csv) |
...
| [artifact-library.html](reference/artifact-library.html) | Every page published for RackTrack, grouped by the piece of work it belongs to rather than by date: what each one contains, which link is current, and what the older versions were as phases. Covers the 47 pages from 31 August to 18 September and the August cloud study. [Artifact](https://claude.ai/artifact/6dgM28PfLUywkDEvSMoP9P) |
