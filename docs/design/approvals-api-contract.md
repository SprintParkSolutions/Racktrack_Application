...
approval_changes: id, org_id, tenant_id, plan_id, attempt, rack_id, rack_name, item_uid, object_type, object_name, netbox_id, netbox_url, action (create, update, rebind, check; a refusal keeps the action that was asked for), field (`*` for a whole record), before (json), after (json), internal (0/1, RackTrack's own link fields), result (written, failed, verified, mismatch), reason, source (scan, suggestion, manual), rule, approved_by, approved_by_id, approved_at, written_by (`system` for a write an approval started), written_by_id, written_at, incident_number, incident_sys_id. Append only, by triggers; no foreign key, so removing an organization deletes its rows by hand after its plans. The verdict of the check after a write is a row of its own (action `check`), never an update.
...
- GET `/me` -> `{ user: { id, username, email, role, orgId, tenantId }, can: { triage, assign, reassign, manage, approve, write, verify, audit, admin, spoc, registry } }` (triage, assign and reassign are admin only; manage is admin or site manager and gates maintenance windows, exceptions and reports, which a site manager keeps; spoc is true for the SPOC of a Site or the holder of an open check)
- GET `/queue` -> role-specific: `{ sections: [{ key, title, plans: [...] }] }` (a SPOC: spoc "Assigned to me", first; technician: mine, and verification_pending for their Site, empty or not; admin: triage "Needs an admin", rework "Sent back for rework", approval_pending "Waiting for a second approval", write_failed, manual_review, sla_breached; site manager: the same sections less rework, for their own Site and to read, then mine, the rest of their Site's open checks; nobody lost a section they had before the SPOC change; approver: approval_pending; assignee user: assigned, accepted, in_progress, pending; auditor: recent)
- GET `/dashboard` -> counts by status, priority, sla state, plus `{ filters }` for each count so the UI opens the exact list
- GET `/plans` (also `holder=me`; rows carry `holder`, `sender`, `incidentNumber`, `incidentUrl`), GET `/plans/:id` (plan, which carries `siteName` as a list row does; items, tickets, decisions, verifications, comments, events, sla, holder, siteSpoc, sender, incident, incidentStates, suggestions, overrides, changes, rackContact, can)
...
- GET `/notifications` (mine), POST `/notifications/:id/read`, GET `/notifications/prefs`, PUT `/notifications/prefs` (the switches at the top level of the body, `{ email, <event>: bool }`; a body that carries none is a 400, never a save)
