...
describe('<HomePage> by role', () => {
  ...
  test('the banner is the role, in words', () => {
    ...
    /* An admin's card introduces the product and ends with their own estate,
       rather than passing a verdict on it (the owner, 23 September 2026). */
    const calm = bannerFor({ role: 'admin', triage: 0, racks: 4, org: 'DC-007', sites: 2, open: 9 });
    expect(calm.title).toBe('One photograph, and the rack is read');
    expect(calm.words).toMatch(/checked against your records/);
    expect(calm.words).toMatch(/2 sites, 4 racks read/);
    // And before anything has been photographed it says so in the same breath.
    expect(bannerFor({ role: 'admin', triage: 0, racks: 0 }).words)
      .toMatch(/Nobody has photographed a rack yet/);
