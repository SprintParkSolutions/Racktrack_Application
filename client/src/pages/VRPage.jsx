...
export default function VRPage() {
  ...
  const msg = {
    ...
    gap:14, padding:'28px 20px', textAlign:'center', color:'#b8b8bc',
  };
    ...
    <Shell onClose={() => navigate(-1)} view={view} setView={setView}
      ...
      {view === 'topology' && (
        topo ? (
          ...
          <div style={msg}>
            <div style={{fontSize:36}}>⚠️</div>
            <p style={{fontSize:14, color:'#f2f2f4'}}>Couldn't load topology</p>
...
function Shell({ children, onClose, view, setView, hint, hideToggle }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'#1a1c1d', zIndex:1000 }}>
      {children}
      ...
      <div style={{
        ...
        <div style={{
          ...
          color:'#f2f2f4', textTransform:'uppercase',
            ...
            {VIEWS.map(v => (
              <button key={v.key} type="button"
                ...
                style={{
                  ...
                  color: view === v.key ? '#ffffff' : 'rgba(255,255,255,0.55)',
                  background: view === v.key ? 'rgba(0,0,0,0.28)' : 'transparent',
                  boxShadow: view === v.key ? '0 0 14px rgba(0,0,0,0.3)' : 'none',
...
function Photo3DView({ imageUrl }) {
  if (!imageUrl) {
    return (
      <div style={{
        ...
        justifyContent:'center', color:'#b8b8bc', fontSize:13,
  ...
  return (
    <Canvas camera={{ position:[1.6, 1.4, 2.6], fov:55, near:0.05, far:60 }}
      ...
      <color attach="background" args={['#1a1c1d']} />
      <fog attach="fog" args={['#1a1c1d', 6, 18]} />
      <ambientLight intensity={0.55} />
      <directionalLight position={[3, 5, 4]} intensity={0.9} />
      <hemisphereLight args={[0x9bb1ff, 0x1f2937, 0.35]} />
      <gridHelper args={[24, 48, '#1a1c1d', '#000000']} />
  ...
  return (
    <group>
      ...
      <mesh position={[0, yCenter, 0]}>
        ...
        <meshStandardMaterial color="#1a1c1d" metalness={0.35} roughness={0.7} />
      </mesh>
...
function Rack3DView({ imageUrl, devices }) {
  ...
  if (error) {
    return (
      <div style={{
        ...
        color:'#f2f2f4', fontSize:13, textAlign:'center',
      }}>
        <div style={{fontSize:30}}>⚠️</div>
        <p>Couldn't crop devices from photo</p>
        <p style={{color:'#b8b8bc', fontSize:12}}>{error}</p>
      </div>
    );
  }
  if (!crops) {
  ...
  if (crops.length === 0) {
    return (
      <div style={{
        position:'absolute', inset:0, display:'flex', flexDirection:'column',
        alignItems:'center', justifyContent:'center', gap:8, padding:20,
        color:'#b8b8bc', fontSize:13, textAlign:'center',
      }}>
  ...
  return (
    <Canvas camera={{ position:[2.0, 1.4, 3.0], fov:55, near:0.05, far:60 }}
            ...
            style={{ position:'absolute', inset:0, touchAction:'none' }}>
      <color attach="background" args={['#1a1c1d']} />
      <fog attach="fog" args={['#1a1c1d', 8, 22]} />
  ...
  return (
    <group>
      ...
      <mesh position={[-rackW / 2 - 0.04, rackH / 2, -0.05]}>
        ...
        <meshStandardMaterial color="#1a1c1d" metalness={0.5} roughness={0.55} />
      </mesh>
      <mesh position={[rackW / 2 + 0.04, rackH / 2, -0.05]}>
        <boxGeometry args={[0.05, rackH * 1.03, 0.32]} />
        <meshStandardMaterial color="#1a1c1d" metalness={0.5} roughness={0.55} />
...
function DeviceBox({ crop, position, size }) {
  ...
  const darkMat = useMemo(
    () => new THREE.MeshStandardMaterial({ color: '#000000', roughness: 0.85 }),
    []
  );
...
function Real3DView({ scanId }) {
    ...
    return (
      <>
        <Canvas
          ...
          style={{ position:'absolute', inset:0, touchAction:'none' }}>
          <color attach="background" args={['#1a1c1d']} />
          <fog attach="fog" args={['#1a1c1d', 8, 30]} />
        ...
        <div style={{
          ...
          <span style={{
            ...
            color:'#f9f9fb', fontSize:10, fontWeight:600,
          ...
          <button onClick={clearMesh}
            style={{
              ...
              border:'1px solid rgba(255,255,255,0.12)',
              borderRadius:999, padding:'6px 12px',
              color:'#cfc4c5', fontSize:11, fontWeight:700, letterSpacing:'0.06em',
  ...
  return (
    <div style={{
      ...
      <p style={{
        fontSize:13, color:'#f9f9fb', maxWidth:360, lineHeight:1.5,
      ...
      <ol style={{
        fontSize:12, color:'#b8b8bc', maxWidth:360, lineHeight:1.6,
        paddingLeft:20, margin:0,
      }}>
        <li>On iPhone 17 Pro: open <b style={{color:'#cfc4c5'}}>Polycam</b> (App Store, free) or <b style={{color:'#cfc4c5'}}>Reality Composer</b>.</li>
        <li>Scan the rack — Polycam: "LiDAR" mode; Reality Composer: "Object Capture". ~30–60 s.</li>
        <li>Export → <b style={{color:'#cfc4c5'}}>glTF / GLB</b> (recommended). USDZ also works in Apple apps but not here yet.</li>
      ...
      {error && (
        <p style={{ fontSize:12, color:'#f2f2f4', textAlign:'center', maxWidth:360 }}>
          {error}
        </p>
      )}

      <p style={{
        fontSize:11, color:'#b8b8bc', textAlign:'center', maxWidth:360,
...
function SidesUploader({ sides, frontFallback, busy, error, onPickSide, onPickVideo, onDone, canDone }) {
  ...
  return (
    <div style={{
      ...
      <p style={{
        fontSize:13, color:'#f9f9fb', maxWidth:340, lineHeight:1.5,
        textAlign:'center', margin:0,
      }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8, width:'100%', maxWidth:360 }}>
        ...
        <p style={{
          fontSize:11, color:'#b8b8bc', textAlign:'center', margin:0, lineHeight:1.4,
        }}>
      ...
      {error && (
        <p style={{ fontSize:12, color:'#f2f2f4', textAlign:'center', maxWidth:340 }}>
          {error}
        </p>
      )}
...
function SideSlot({ side, image, isFallback, busy, onPick }) {
  return (
    <label style={{
      ...
      border: image ? '1px solid rgba(0,0,0,0.35)' : '1px dashed rgba(255,255,255,0.18)',
...
function TexturedRack({ sides }) {
  ...
  const materials = useMemo(() => ([
    ...
    new THREE.MeshStandardMaterial({ color: '#1a1c1d' }),
    new THREE.MeshStandardMaterial({ color: '#1a1c1d' }),
